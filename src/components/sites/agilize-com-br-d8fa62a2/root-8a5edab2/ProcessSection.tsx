...
const STEPS_BY_TAB: Record<(typeof TABS)[number]["id"], { icon: typeof Headphones; title: string; description: string }[]> = {
  abrir: [
    { icon: Headphones, title: "Consultoria especializada", description: "Nosso time de contadores ajuda você a escolher o melhor regime tributário e estrutura para sua empresa." },
    { icon: FileCheck2, title: "Abertura da sua empresa", description: "Assumimos toda a parte burocrática e iniciamos o processo de abertura, atualizando você sobre cada etapa." },
    { icon: CheckCircle2, title: "Seu CNPJ está pronto!", description: "Após a validação, sua empresa está aberta e nós assumimos a contabilidade para você focar em crescer." },
  ],
  trocar: [
    ...
    <section className="py-20">
      <div className="mx-auto w-full max-w-[1440px] px-6 lg:px-10">
        ...
        <p className="mx-auto mt-4 max-w-2xl text-center text-muted-foreground">
          Do melhor regime tributário à rotina contábil do dia a dia, veja
          como é simples abrir sua empresa ou migrar sua contabilidade.
        </p>

        <div className="mt-10 rounded-3xl bg-secondary p-6 sm:p-10">
          <div
            role="tablist"
          ...
          <div className="mt-8 grid grid-cols-1 gap-8 rounded-2xl bg-background p-6 sm:p-10 md:grid-cols-3">
            {steps.map((step, i) => (
              <div key={step.title} className="text-center">
                <div className="relative mx-auto flex size-16 items-center justify-center rounded-full bg-primary text-primary-foreground">
                  ...
                  <span className="absolute -right-1 -top-1 flex size-6 items-center justify-center rounded-full bg-muted text-xs font-bold text-foreground">
                    {i + 1}
                  </span>
                </div>
